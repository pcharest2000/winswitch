...
gboolean labelMatch(char input) { return TRUE; }
void gtkInitWindow() {
...
void calFontPosition() {
  ...
  void returnHome() {
    ...
    xcb_warp_pointer(xcb_con, XCB_NONE, currentActiveWin, 10, 10, 20, 20, 20,
                     20);
    xcb_ewmh_set_current_desktop(ewmh_con, 0, curentActiveDesktop);
    requestWindowChange(currentActiveWin);
    xcb_flush(xcb_con);
    usleep(100); // This is required when quitting sometimes does not honor
  }
  ...
  void getVisibleWindows() {
    ...
    xcb_ewmh_get_current_desktop_reply(
        ewmh_con, xcb_ewmh_get_current_desktop(ewmh_con, 0),
    ...
    for (uint32_t i = 0; i < windowsReply.windows_len; i++) {
      ...
      if (state == XCB_MAP_STATE_VIEWABLE) {
        ...
        xcb_ewmh_get_wm_desktop_reply(
            ewmh_con,
            xcb_ewmh_get_wm_desktop(ewmh_con, windowsReply.windows[i]), &desk,
            NULL);
        ...
        Window junkroot;
        XGetGeometry(disp_con, windowsReply.windows[i], &junkroot, &junkx,
                     &junky, &wwidth, &wheight, &bw, &depth);
        XTranslateCoordinates(disp_con, windowsReply.windows[i], junkroot,
                              junkx, junky, &x, &y, &junkroot);
  ...
  void printDesktopInfo() {
    ...
    for (uint32_t i = 0; i < numVisibleDesktops; i++) {
      printf("\nDesk info: D: %d  WT:  %d  ind: %d Char %c",
             visibleDesktopsArray[i].desktopNum,
             visibleDesktopsArray[i].numWindows,
             visibleDesktopsArray[i].indexWindow,
             visibleDesktopsArray[i].firstChar);
    }
  }
  void labelWindows() {
    ...
    if (numVisibleDesktops == 1) {
      if (numVisibleWindows == 1) {
        ...
        visibleWindowsArray[0].charWidth = 1;
        visibleWindowsArray[0].as[0] = labelString[0];
        visibleWindowsArray[0].as[1] = '\0';
        return;
      }
    ...
    if (numVisibleDesktops > 1) {
      ...
      for (uint32_t k = 0; k < numVisibleDesktops; k++) {
        ...
        uint32_t endWindow =
            visibleDesktopsArray[k].numWindows + startingWindow;
